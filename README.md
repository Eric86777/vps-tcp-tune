# BBR v3 优化脚本 - Ultimate Edition v4.8.2
...
- **三选一模式**：
  ...
  - `2. 纯国内模式`：阿里云 + 腾讯 DNSPod，无备用，无加密（国内DNS不支持DoT）
  - `3. 混合模式`：国外首选 + 国内备用，机会性 DoT，最大容错
- **安全说明**：v4.8.2 已内置完整的事务性回滚机制（执行前全量快照 → 任意步骤失败自动恢复原始状态），重启持久化也已修复。如仍有顾虑，建议在有 VNC/控制台的情况下首次使用。
