...
| **5** | NS论坛-DNS净化 | 抗污染、驯服DHCP、修复 systemd-resolved |
| **6** | **Realm转发timeout修复** | ⭐ **推荐**，解决中转断流问题 |
| **7** | NS论坛CAKE调优 | 降低网络延迟，对抗缓冲区膨胀 |
| **8** | 科技lion高性能模式 | 针对 Reality/VLESS 的极限参数调优 |

### 🟡 系统配置
| 编号 | 功能名称 | 说明 |
|------|---------|------|
| **9** | 设置IPv4/IPv6优先级 | 解决 Google 验证码跳验证等问题 |
| **10** | IPv6管理 | 临时/永久禁用或恢复 IPv6 |
| **11** | 设置临时SOCKS5代理 | 终端临时走代理，支持认证 |
| **12** | 虚拟内存管理 | 智能计算并添加 Swap，防止 OOM |
| **13** | 查看系统详细状态 | CPU/内存/磁盘/网络/内核信息 |

### 🟣 转发/代理配置
| 编号 | 功能名称 | 说明 |
|------|---------|------|
| **14** | Realm转发连接分析 | 实时监控转发流量与来源 IP |
| **15** | **Realm强制使用IPv4** | ⭐ **推荐**，提升转发稳定性 |
| **16** | IPv4/IPv6连接检测 | 测试双栈连通性 |
| **17** | 查看Xray配置 | 显示当前节点配置信息 |
| **18** | 设置Xray IPv6出站 | 让 Xray 优先走 IPv6 |
| **19** | 恢复Xray默认配置 | 救砖功能 |

### 🟠 代理部署
| 编号 | 功能名称 | 说明 |
|------|---------|------|
| **20** | **星辰大海Snell协议** | ⭐ **推荐**，v5.0.1内核，支持多实例/多端口 |
| **21** | **星辰大海Xray一键双协议** | ⭐ **推荐**，VLESS+Reality 极简部署 |
| **22** | 禁止端口通过中国大陆直连 | 安全防护，防止被扫 |
| **23** | 一键部署SOCKS5代理 | 快速搭建 SOCKS5 服务 |
| **24** | Sub-Store多实例管理 | 强大的订阅转换工具 |
| **25** | **一键反代 🎯** | ⭐ **推荐**，Cloudflare Tunnel 内网穿透 |

### 🔴 测试检测
| 编号 | 功能名称 | 说明 |
|------|---------|------|
| **26** | IP质量检测（IPv4+IPv6） | 综合欺诈分数检测 |
| **27** | **IP质量检测（仅IPv4）** | ⭐ **推荐**，快速检测 |
| **28** | 服务器带宽测试 | Speedtest 测速 |
| **29** | iperf3单线程测试 | 精准测试网络吞吐量 |
| **30** | **国际互联速度测试** | ⭐ **推荐**，全球节点测速 |
| **31** | **网络延迟质量检测** | ⭐ **推荐**，丢包率与延迟抖动 |
| **32** | **三网回程路由测试** | ⭐ **推荐**，检测线路质量（CN2/9929/CMIN2） |
| **33** | **IP媒体/AI解锁检测** | ⭐ **推荐**，Netflix/Disney+/ChatGPT 等 |
| **34** | NS一键检测脚本 | 综合系统信息检测 |

### 🟤 第三方工具
| 编号 | 功能名称 | 说明 |
|------|---------|------|
| **35** | PF_realm转发脚本 | 强大的端口转发工具 |
| **36** | F佬一键sing box脚本 | 全能代理工具 |
| **37** | 科技lion脚本 | 综合运维脚本 |
| **38** | 酷雪云脚本 | 综合运维脚本 |
...
**项目地址**: [https://github.com/Eric86777/vps-tcp-tune](https://github.com/Eric86777/vps-tcp-tune)
