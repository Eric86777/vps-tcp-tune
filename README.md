...
只需运行脚本并选择菜单项 `99`，即可触发“完全卸载脚本”流程。该选项会自动：
- 卸载 XanMod 内核（如已安装）
- 清理脚本创建的 bbr 快捷别名及相关配置
- 删除所有 sysctl 调优文件与备份
- 还原脚本新增的软件源及临时文件
执行完成后脚本会提示是否需要重启，确保卸载动作干净彻底。若当前终端仍能运行 `bbr`，手动输入 `unalias bbr` 或重新加载 Shell 配置即可立即生效。
...
2. **⚡ BBR TCP调优** - BBR直连/落地优化（智能带宽检测）+ **DNS净化与安全加固（NEW）** + CAKE队列优化 + 3种Reality专用优化（覆盖512MB-8GB+所有配置）
...
8. **🚀 代理部署** - 一键部署 SOCKS5 代理（基于 Sing-box） + Sub-Store 多实例管理 + **一键反代 🎯（NEW）**
...
适用场景：VPS提供商预装DNS污染、DHCP强制覆盖DNS、systemd-resolved被屏蔽等问题。

作用摘要：
- 🔓 **智能修复 systemd-resolved**：自动检测并解除 masked 状态（DMIT等VPS常见问题）
- 🛡️ **DNS over TLS (DoT)**：启用加密DNS查询，防止DNS劫持
- 🔒 **DNSSEC验证**：启用DNS安全扩展，防止DNS欺骗
- 🚫 **驯服DHCP客户端**：强制忽略DHCP推送的DNS服务器
- 🧹 **清理厂商残留**：移除 `/etc/network/interfaces` 中的DNS配置
- ✅ **安全DNS服务器**：使用 Google DNS (8.8.8.8) + Cloudflare DNS (1.1.1.1)

核心改进（相比原版）：
- ✅ **自动解除 systemd-resolved 屏蔽**（原版会直接失败）
- ✅ **智能检测服务状态**（masked/disabled/inactive）
- ✅ **完整的错误处理**（修复失败会明确提示）
- ✅ **验证服务运行状态**（确保配置生效）
...
**⚠️ 注意事项**：
- 会重启 networking.service（SSH连接可能短暂中断）
...
<details>
<summary>🎯 一键反代 - 通用反向代理管理（NEW）</summary>

**菜单位置**：选项 **24**

**核心功能**：
- 🚀 **超简单部署**：只需输入端口和域名，3分钟完成反代
- 🔒 **基于 Cloudflare Tunnel**：无需开端口，自动 HTTPS，隐藏源 IP
- 🎯 **通用性强**：支持任何 HTTP/HTTPS 服务（Flux-Panel、X-UI、自定义应用等）
- 📊 **配置管理**：查看、删除已部署的反代配置
- 🛡️ **安全防护**：享受 Cloudflare 的 DDoS 防护
...
1. **反代 Flux-Panel**
   ...
   端口: 6366
   域名: flux.example.com
   访问: https://flux.example.com
   ```

2. **反代自定义项目**
   ```
   端口: 5555
   域名: project.example.com
   访问: https://project.example.com
   ```
...
3. **反代 X-UI 面板**
   ...
   端口: 54321
   域名: xui.example.com
   访问: https://xui.example.com
   ```

**管理功能**：
- **查看反代**：显示所有已部署的反代配置和状态
- **删除反代**：停止服务、删除配置、可选删除隧道
...
**配置文件位置**：
```bash
/root/reverse-proxy-configs/
├── config.json                    # 主配置文件
└── cf-tunnel/
    └── tunnel-*.yaml              # CF Tunnel 配置
```

**服务管理**：
```bash
# 查看状态
systemctl status cloudflared-tunnel-<隧道名称>

# 查看日志
journalctl -u cloudflared-tunnel-<隧道名称> -f

# 重启服务
systemctl restart cloudflared-tunnel-<隧道名称>
```

**优势对比**：

| 特性 | 一键反代 (CF Tunnel) | Nginx/Caddy | Frp/Nps |
|------|---------------------|-------------|---------|
| **需要开端口** | ❌ 不需要 | ✅ 需要 80/443 | ✅ 需要 |
| **隐藏源 IP** | ✅ 完全隐藏 | ❌ 暴露 | ⚠️ 部分隐藏 |
| **SSL 证书** | ✅ 自动 | ⚠️ 需配置 | ⚠️ 需配置 |
| **DDoS 防护** | ✅ CF 自带 | ❌ 无 | ❌ 无 |
| **配置难度** | ⭐ 超简单 | ⭐⭐⭐ 中等 | ⭐⭐⭐⭐ 复杂 |
| **性能** | ⭐⭐⭐ 有延迟 | ⭐⭐⭐⭐⭐ 直连 | ⭐⭐⭐⭐ 较好 |
